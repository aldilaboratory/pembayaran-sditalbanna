<x-super_admin.layout>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-body">
          ...
          {{-- FORM simpan tagihan --}}
          <form action="{{ route('super_admin.data_tagihan_siswa.store') }}" method="POST" id="feeForm">
            @csrf
            ...
            <div class="mt-3">
              <a href="{{ route('super_admin.data_tagihan_siswa') }}" class="btn btn-light">Kembali</a>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Fungsi untuk mengecek duplikasi tagihan
       async function checkDuplicateTagihan() {
         ...
         try {
           const response = await fetch('{{ route("super_admin.data_tagihan_siswa.check_duplicate") }}', {
             method: 'POST',
             headers: {
    ...
    classSelect.addEventListener('change', async function(){
      ...
      try {
        const url = "{{ route('super_admin.data_tagihan_siswa.students') }}" + '?class_id=' + encodeURIComponent(classId);
